...
public interface CustomerQueryConstants {
	...
	String GET_ALL_CATEGORIES = "select r.id,r.name,r.image_path,r.active,r.group_id from rootcategories r,invrootcategories i where r.id = i.rootcategory_id\r\n" + 
	...
	String UPDATED_CUSTOMER_PROFILE = "update customers set customers_firstname=?,customers_dob=STR_TO_DATE(?,'%Y-%m-%d'),customers_email_address=?,customers_password=? where id = ?";
	...
	String GET_STORE_DETAILS = "select p.id,p.business_name,p.business_short_desc,p.user_image,p.business_long_desc,p.business_operating_mode,p.physical_store_status,\r\n" + 
			...
			"d.min_order_amount,d.delivery_charge,p.store_tax_status,p.store_tax_percentage,p.store_tax_gst,p.status,d.free_delivery_above,p.app_version,p.dunzo_flag,p.store_latitude,p.store_longitude,p.delivery_radius,p.store_pickup,p.store_fssai,d.delivery_percentage  from profiles p,deliveries d where p.id = d.store_id \r\n" + 
			"and p.id = ?";
	
	String GET_STORE_DELIVERY_AREAS = "select d.delivery_areas,p.delivery_radius from profiles p,deliveries d where p.id = d.store_id and p.id = ?";
	
	String GET_STORE_ROOT_CATEGORY="SELECT r.name,r.image_path,r.active,i.rootcategory_id,i.id,r.group_id FROM rootcategories r,invrootcategories i where r.id = i.rootcategory_id and i.store_id = ? and i.active = ?";
	...
	String GET_PRODUCT_DETAILS_BY_ID = "select p.id,p.name,p.category_id,p.description,p.image_path,i.active,i.product_id,i.store_id,i.price,\r\n" + 
			"i.special_price,i.promotional_price,i.weight,i.color,i.`size`,i.quantity,i.option1,i.option2,i.diet_category from products p,invproducts i where p.id = i.product_id and p.id = ? and i.store_id = ? and i.branch_id = 1";
	...
	String GET_PRODUCT_DETAILS_BY_STOREID = "select p.id,p.name,p.category_id,p.description,p.image_path,i.active,i.product_id,i.store_id,i.price,\r\n" + 
			"i.special_price,i.promotional_price,i.weight,i.color,i.`size`,i.quantity,i.option1,i.option2,i.diet_category from products p,invproducts i where p.id = i.product_id and i.store_id = ? and p.name like ?";
	...
	String GET_PRODUCT_DETAILS_BY_CATEGORY= "select p.id,p.name,p.category_id,p.description,p.image_path,i.active,i.product_id,i.store_id,i.price,\r\n" + 
			"i.special_price,i.promotional_price,i.weight,i.color,i.`size`,i.quantity,i.option1,i.option2,i.diet_category from products p,invproducts i where p.id = i.product_id and i.store_id = ? and i.category_id = ?";
	...
	String ADD_DEVICETOKEN = "INSERT INTO user_devicetoken (user_id, device_token, device_type, user_type, created_at, updated_at, one_signal_id) VALUES(?, ?, ?, ?, current_timestamp, current_timestamp, ?)";
	
	String UPDATE_DEVICE_TOKEN = "UPDATE user_devicetoken SET device_token=?, device_type=?, user_type=?,updated_at=current_timestamp, one_signal_id = ? WHERE user_id=?";
	...
	String GET_CUSTOMER_ADDRESS_BYID = "SELECT id, customer_id, address_label, door_no, street_name, city_name, pin_code, state, country, address_latitude, address_longitude, created_at, updated_at, deleted_at\r\n"
			+ "FROM customer_addresses WHERE id=?";
	
	public String GET_CUSTOMER_MOBILE = "select c.customers_telephone from customers c where c.id = ?";
	
	String UPDATE_ONESIGNAL_ID = "UPDATE user_devicetoken SET one_signal_id=?, updated_at=current_timestamp WHERE user_id=?";
	
	String GET_ONESIGNAL_ID = "select one_signal_id from user_devicetoken WHERE user_id=?";
	
	String GET_BUSINESS_ONESIGNAL_ID = "select ud.one_signal_id  from user_devicetoken ud inner join users u on u.id = ud.user_id and u.store_id = ?";
	
	String GET_DUNZO_FLAG = "select dunzo_flag from profiles where id = ?";
	
	String GET_ADBANNER_DETAILS = "select id,store_id,image_path,ad_url from ad_banners where store_id=?";
	
	public String GET_SEGMENTS = "select id,name,description,store_id,active from segments where store_id = ?";
	
	public String GET_SEGMENT_PRODUCTS = "select product_id from segment_products where segment_id = ?";
	
	public String GET_PRODUCT_DETAILS= "select p.id,p.name,p.category_id,p.description,p.image_path,i.active,i.product_id,i.store_id,i.price,\r\n" + 
			"i.special_price,i.promotional_price,i.weight,i.color,i.`size`,i.quantity,i.option1,i.option2,i.diet_category from products p,invproducts i where p.id = i.product_id and i.product_id = ? and i.branch_id = 1";

	public String GET_SEGMENT_BY_ID = "select id,name,description,store_id,active from segments where id = ?";
}
